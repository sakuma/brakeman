...
class Brakeman::Scanner
  ...
  def parse_files
    if @options[:parallel_file_read]
      fp = Brakeman::ParallelFileParser.new tracker, @app_tree
    else
  ...
  def process_controllers
    track_progress @file_list[:controllers] do |controller|
      ...
      process_controller controller.ast, controller.path
    end
  end
  ...
  def process_controller ast, path
    begin
      @processor.process_controller(ast, path)
    rescue => e
      tracker.error e.exception(e.message + "\nWhile processing #{path}"), e.backtrace
    end
  end
  ...
  def process_templates
    ...
    track_progress templates, "templates" do |t|
      Brakeman.debug "Processing #{t[:path]}"
      process_template t[:path], t[:ast], t[:name], t[:type]
    end
  end

  def process_template path, ast, name, type
    @processor.process_template(name, ast, type, nil, path)
  end
